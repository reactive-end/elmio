...
export class FileEnterpriseRepositoryService implements EnterpriseRepositoryPort {
  ...
  async updateDelinquencyBucket(
    ...
    // Implementacion file-based: edita el storage JSON.
    // (No se usa en produccion porque el DbEnterpriseRepository es el real.)
    const data = await this.read();
    const idx = data.purchases.findIndex((p) => p.id === purchaseId);
    if (idx >= 0) {
      data.purchases[idx] = {
        ...data.purchases[idx],
