/**
 * @fileoverview Componente de Paso 1: Datos del Asegurado para La Mundial de Seguros.
 * @description Captura los datos personales del titular usando CedulaInput y PhoneInput.
 * @module components/molecules/MundialRCVSteps/Step1InsuredData
 */
...
import type { InsuredData } from '@/src/hooks/pages/useMundialConsultaRCV'
      ...
      <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
        ...
        <div>
          ...
          <p className="text-xs text-body-muted">
            Ingrese la información personal del titular de la póliza de La Mundial.
          </p>
        </div>
      </div>
      ...
      <form className="grid grid-cols-1 sm:grid-cols-2 gap-4" onSubmit={onSubmit}>
        ...
        <FormField label="Cédula" required>
          <CedulaInput
            ...
            onChange={(v) => setInsured((p) => ({ ...p, docType: v.letter, docNumber: v.digits }))}
            allowedLetters={['V', 'E']}
          />
        </FormField>
        ...
        <FormField label="Teléfono" required>
          <PhoneInput
            ...
            onCountryCodeChange={() => {}}
            operatorPrefix={
              insured.phoneCode.slice(1) as '412' | '422' | '414' | '424' | '416' | '426'
            }
            onOperatorPrefixChange={(prefix) =>
              setInsured((p) => ({ ...p, phoneCode: `0${prefix}` }))
            }
            hideCountrySelector
          />
        </FormField>
        ...
        <FormField label="Género" required>
          <Select
            ...
            onChange={(v) => setInsured((p) => ({ ...p, genderId: v }))}
            options={[
