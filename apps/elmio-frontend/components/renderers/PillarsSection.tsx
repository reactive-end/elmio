      ...
      <div className="container mx-auto px-4">
        {(contenido.titulo || contenido.subtitulo) && (
          <div className="mb-10 text-center">
            {contenido.titulo && (
              <h2 className="mb-2 text-2xl font-bold md:text-3xl" style={{ color: estilo.tituloColor }}>
                {contenido.titulo}
              </h2>
            )}
            {contenido.subtitulo && (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {pilares.map((pilar, idx) => (
            <div key={pilar.id ?? idx} className="flex flex-col items-center rounded-2xl bg-gray-50 p-6 text-center transition-all hover:shadow-md">
              {pilar.icono ? (
                <img src={pilar.icono} alt={pilar.titulo} className="mb-4 h-12 w-12 object-contain" />
              ) : (
              ...
              <h3 className="mb-2 font-semibold" style={{ color: estilo.tituloColor, fontSize: 18 }}>
                {pilar.titulo}
              </h3>
              ...
              {pilar.textoBoton && (
                <button type="button" className="mt-4 rounded-xl px-5 py-2 text-sm font-semibold transition-colors"
