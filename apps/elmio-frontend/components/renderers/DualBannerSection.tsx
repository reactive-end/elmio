      ...
      <div className="container mx-auto px-4">
        <div className="grid gap-6 md:grid-cols-2">
          {banners.map((banner, idx) => (
            <div key={banner.id ?? idx} className="group relative overflow-hidden rounded-2xl" style={{ minHeight: 280 }}>
              {banner.imagenUrl ? (
                <img src={banner.imagenUrl} alt={banner.titulo} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
              ) : (
                <div className="absolute inset-0 bg-gradient-to-br from-secondary to-secondary-dark" />
              )}
              <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8" style={{ color: estilo.tituloColor || '#fff' }}>
                <h3 className="text-xl font-bold md:text-2xl">{banner.titulo}</h3>
                {banner.descripcion && <p className="mt-2 text-sm opacity-90">{banner.descripcion}</p>}
                {banner.textoBoton && banner.enlaceBoton && (
                  <Link href={banner.enlaceBoton}
                    ...
                    style={{ backgroundColor: estilo.botonColorFondo || '#fff', color: estilo.botonColorTexto || '#111827', borderRadius: estilo.botonRedondez || 12 }}>
                    {banner.textoBoton}
                  </Link>
                )}
              </div>
