    ...
    <SectionContainer estilo={estilo}>
      <div className="container mx-auto px-4">
        ...
        <div className="relative overflow-hidden">
          <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-1/6" style={{
            background: `linear-gradient(to right, ${estilo.colorFondo !== 'transparente' ? estilo.colorFondo : '#f8fafc'}, transparent)`,
          }} />
          <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-1/6" style={{
          ...
          <div className="flex gap-6 py-4" style={{ animation: 'marquee 40s linear infinite' }}>
            {[...aliados, ...aliados, ...aliados, ...aliados].map((aliado, idx) => (
              <Link key={`${aliado.id}-${idx}`} href={aliado.href || '#'}
                ...
                {aliado.logo ? (
                  <img src={aliado.logo} alt={aliado.nombre} className="max-h-12 max-w-full object-contain transition-all" />
                ) : (
