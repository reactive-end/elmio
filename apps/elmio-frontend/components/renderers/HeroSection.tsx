...
export function HeroSection({ seccion }: HeroSectionProps) {
  ...
  const slides = contenido.diapositivas.length > 0 ? contenido.diapositivas : [
    { id: 'default-hero', imagen: contenido.imagenUrl, titulo: contenido.titulo, subtitulo: contenido.subtitulo, texto: contenido.descripcion, textoBoton: contenido.textoBoton, enlaceBoton: contenido.enlaceBoton },
  ...
  return (
    <SectionContainer estilo={{ ...estilo, paddingSuperior: 0, paddingInferior: 0, paddingDerecho: 0, paddingIzquierdo: 0 }}>
      <div className="relative flex h-[300px] items-center justify-center overflow-hidden md:h-[520px]">
        ...
        >
          {slides.map((slide, idx) => (
            <div key={slide.id ?? idx} className="h-full w-full shrink-0 relative">
              {slide.imagen ? (
                <img src={slide.imagen} alt={slide.titulo ?? `Slide ${idx + 1}`} className="h-full w-full object-cover" />
              ) : (
        ...
        <div className="relative z-10 flex w-full max-w-4xl flex-col px-8 py-16" style={{ alignItems: 'center', textAlign: 'center' }}>
          {slideActual.titulo && (
            <h1 className="mb-3 font-bold text-white drop-shadow-lg" style={{ fontSize: estilo.tituloTamano, fontWeight: estilo.tituloPeso }}>
              {slideActual.titulo}
            </h1>
          )}
          {slideActual.subtitulo && (
            <p className="mb-2 text-lg text-white/80 drop-shadow" style={{ fontSize: estilo.subtituloTamano }}>
              {slideActual.subtitulo}
            </p>
          )}
          {slideActual.texto && (
            <p className="mb-6 max-w-2xl text-white/70 drop-shadow" style={{ fontSize: estilo.cuerpoTamano }}>
              {slideActual.texto}
            </p>
          )}
        ...
        {slides.length > 1 && (
          <>
            ...
            <div className="absolute bottom-6 left-1/2 z-20 flex -translate-x-1/2 gap-2">
              {slides.map((_, idx) => (
                <button key={idx} onClick={() => { setIndiceActual(idx); iniciarAuto() }}
