...
import { Landmark, Upload, Loader2, Check, Eye, Trash2 } from 'lucide-react'
...
const VENEZUELAN_BANKS = [
  ...
  { value: '0104', label: 'Banco Venezolano de Crédito (0104)' },
  ...
  { value: '0166', label: 'Banco Agrícola de Venezuela (0166)' },
  ...
  { value: '0191', label: 'Banco Nacional de Crédito (0191)' },
]

/**
 * Onboarding bancario para personas naturales y colaboradores.
 * Paso unico: registrar datos bancarios obligatorios.
 */
export default function PersonOnboardingPage() {
  ...
  // Telefono con PhoneInput + usePhoneFormat
  ...
  const handlePhotoChange = useCallback((files: FileList) => {
    ...
    try {
      ...
      setError('Error al subir la foto de la cedula.')
      setPhotoUploading(false)
    }
  }, [])
  ...
  const handlePreviewPhoto = useCallback(() => {
    if (documentPhoto) {
      setPhotoPreviewUrl(documentPhoto)
    }
  }, [documentPhoto])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al registrar cuenta bancaria.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="w-full max-w-2xl">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-blue-50 mb-4">
            <Landmark className="w-8 h-8 text-blue-600" strokeWidth={1.5} />
          </div>
          <h1 className="text-2xl font-bold text-body">Registro Bancario</h1>
          <p className="text-sm text-body-muted mt-1">Completa tus datos bancarios para continuar en ElMio.</p>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField label="Banco" required>
                <Select
                  ...
                  hasError={validationErrors.bankCode}
                />
              </FormField>
                ...
                {photoUploading ? (
                  ...
                  <div className="flex items-center justify-between border border-green-200 bg-green-50/50 rounded-xl p-4 w-full">
                    <div className="flex items-center gap-3 overflow-hidden">
                      ...
                      <span className="text-sm font-semibold text-green-800">Foto cargada con exito</span>
                    </div>
